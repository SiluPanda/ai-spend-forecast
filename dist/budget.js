...
exports.alertOnBudget = alertOnBudget;
 ...
 */
function alertOnBudget(history, budget, options) {
    if (!budget || budget.amount <= 0) {
        throw new errors_js_1.ForecastError('INVALID_BUDGET', 'Budget amount must be a positive number');
    }
    const records = (0, normalize_js_1.normalizeHistory)(history);
    // Determine budget period
    const now = new Date();
    const periodStart = budget.periodStart ?? (0, normalize_js_1.formatDateOnly)(new Date(now.getFullYear(), now.getMonth(), 1));
    const periodEnd = budget.periodEnd ?? (0, normalize_js_1.formatDateOnly)(new Date(now.getFullYear(), now.getMonth() + 1, 0));
    ...
    const today = new Date((0, normalize_js_1.formatDateOnly)(now));
    ...
    if (daysRemaining > 0 && records.length >= 3) {
        forecastResult = (0, forecast_js_1.forecast)(history, {
            ...options,
