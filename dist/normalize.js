...
exports.extractCosts = extractCosts;
...
function normalizeHistory(input) {
    if (!input || (Array.isArray(input) && input.length === 0)) {
        throw new errors_js_1.ForecastError('INVALID_HISTORY', 'History must not be empty');
    }
    let records;
    if (Array.isArray(input)) {
        ...
        throw new errors_js_1.ForecastError('INVALID_HISTORY', 'Unsupported history format');
    }
    // Validate that each record has a cost
    for (const record of records) {
        if (record.cost === undefined && !record.tokenUsage) {
            throw new errors_js_1.ForecastError('MISSING_COST', `Record with date "${record.date}" has neither cost nor tokenUsage`);
        }
    }
    // Validate chronological sorting
    for (let i = 1; i < records.length; i++) {
        if (records[i].date < records[i - 1].date) {
            throw new errors_js_1.ForecastError('INVALID_HISTORY', `History is not sorted chronologically: "${records[i - 1].date}" appears before "${records[i].date}"`);
        }
    }
    return records;
}
