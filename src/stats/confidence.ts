import { tCritical } from './t-distribution.js';
...
export function zScore(confidenceLevel: number): number {
  if (Z_SCORES[confidenceLevel] !== undefined) return Z_SCORES[confidenceLevel];
 ...
 * @returns { lower, upper }
 */
export function maConfidenceBounds(
  predicted: number,
  stdDev: number,
 ...
 * @returns { lower, upper }
 */
export function regressionPredictionBounds(
  predicted: number,
  ser: number,
  n: number,
  meanX: number,
