 ...
 */
export function tCritical(df: number, confidenceLevel: number): number {
  ...
  for (const knownDf of KNOWN_DFS) {
    if (knownDf <= effectiveDf) lowerDf = knownDf;
  ...
  if (lowerDf === upperDf) return lowerT;
...
function lookupAlpha(df: number, alpha: number): number {
  ...
  // Exact match
  if (row[alpha] !== undefined) return row[alpha];
  ...
  if (lowerAlpha === upperAlpha) return row[lowerAlpha];
...
function zScoreFromAlpha(alpha: number): number {
  ...
  if (zTable[alpha] !== undefined) return zTable[alpha];
 ...
 */
export function tPValue(tStat: number, df: number): number {
